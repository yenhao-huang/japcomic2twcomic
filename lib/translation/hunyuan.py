...
class Translation_Model_Hunyuan:
    ...
    def translate_ocr_output_multi_text(
        ...
        for i in tqdm(range(0, len(bbox_texts), batch_size), desc="批次翻譯", unit="批次"):
            ...
            if non_empty_texts:
                ...
                # 解析輸出
                translated_batch = [self.twcc_polish(text) for text in translated_batch]
    ...
    def translate_multi_text(self, texts: List[str]) -> List[str]:
        ...
        for i, text in enumerate(texts, start=1):
            concat_texts += f"<|{i}|>{text}\n"
        
        # 2. 合併 prompt
        if "{text}" in self.prompt:
            formatted_prompt = self.prompt.format(text=concat_texts)
        else:
            formatted_prompt = f"{self.prompt}\n\n{concat_texts}"
        ...
        # 4. 使用正則表達式解析結果
        results = re.split(r'<[^>]+>', output_text)
        # 移除空字串並去除首尾空白
        results = [r.strip() for r in results if r.strip()]

        # 檢查是否所有文字都被翻譯
        if len(results) != len(texts):
            print(f"警告: 翻譯結果數量不符！預期 {len(texts)} 個，實際得到 {len(results)} 個")
            # 填充缺失的翻譯或截斷多餘的結果
            if len(results) < len(texts):
